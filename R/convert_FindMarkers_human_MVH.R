#' @title convert_FindMarkers_human_MVH
...
convert_FindMarkers_human_MVH <- function(genelist, remove_ribosomal_genes = T, create_ranked_vector = T) {
  ...
  # })  
  fish.human.convert.Z11 <- read.delim('/Users/hunterm/Dropbox/MH_ST/Miranda_R/from_Nate/GRCz11_to_HS.txt')
  ...
  if (length(dups) >= 1) {
    ...
    dups_data <- NULL
    for (dup in dups) {
      data <- merged[merged$Human_Symbol == dup,] %>% arrange(-avg_log2FC)
    ...
    if (create_ranked_vector) {
      vector <- merged_new %>% dplyr::select(Human_Symbol, avg_log2FC) %>% deframe()
      return(vector)
    } else {
      return(merged_new)
    }
    ...
    if (create_ranked_vector) {
      ...
      vector <- merged %>% dplyr::select(Human_Symbol, avg_log2FC) %>% deframe()
      return(vector)
    } else {
      return(merged)
    }
